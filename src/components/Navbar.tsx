    ...
    <nav className="fixed top-0 left-0 right-0 z-[100] liquid-panel !border-t-0 !border-l-0 !border-r-0 !rounded-none">
      <div className="max-w-[1200px] mx-auto px-md flex items-center justify-end h-[64px] max-md:justify-end">
        <div className="hidden max-md:flex mr-auto">
          <Link
            href="/"
            className="font-heading text-xl font-extrabold tracking-[-0.5px] flex items-center gap-sm"
          >
        ...
        <div className="flex items-center gap-md">
          ...
          {session?.user ? (
              ...
              {dropdownOpen && (
                <>
                  ...
                  <div className="absolute top-[calc(100%+var(--spacing-sm))] right-0 w-[220px] bg-bg-secondary border border-border-subtle rounded-md shadow-lg p-sm z-[11] flex flex-col gap-sm">
                    ...
                    <button
                      className="p-sm rounded-sm text-sm font-medium text-status-lost bg-transparent transition-all duration-150 text-left w-full hover:bg-status-lost-bg"
                      onClick={() => signOut()}
                    >
                      {t("logout")}
                    </button>
            ...
            <Link
              href="/auth/signin"
              className="liquid-button-primary px-md py-xs rounded-sm text-sm font-semibold flex items-center justify-center"
            >
              {t("login")}
            </Link>
          )}
